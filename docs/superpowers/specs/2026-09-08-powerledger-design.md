...
- `SetSettings` accepts only tariff, machine profile, idle threshold, sample interval, and retention, each range-checked. No paths or commands travel over the pipe. Before sending any change the App checks the process serving the pipe: its executable must be the one Windows starts for the PowerLedger service (`GetNamedPipeServerProcessId`, then the image of that process against `HKLM\SYSTEM\CurrentControlSet\Services\PowerLedger\ImagePath`), because anyone could create a pipe with this name before the service does. A development run started with `--pipe` skips the check. A refused or failed change comes back to the App as a sentence it shows under the Save that sent it.
...
- The App sends changes only to the installed service, identified by its executable rather than by the pipe's name (§8).

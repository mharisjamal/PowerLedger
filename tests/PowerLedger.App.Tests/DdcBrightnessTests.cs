...
public class DdcBrightnessTests(ITestOutputHelper output)
{
    ...
    [Theory]
    [InlineData(1u, MonitorPowerState.On, 0.6)]
    [InlineData(0u, null, 0.6)]                             // it doesn't say, so it may be on
    [InlineData(2u, MonitorPowerState.Standby, null)]       // it says it isn't on, so its brightness isn't asked
    [InlineData(4u, MonitorPowerState.Off, null)]
    public void A_monitor_says_whether_it_is_on_before_it_is_asked_its_brightness(uint mode, MonitorPowerState? state, double? brightness)
    {
        var dell = new FakeMonitor(Dell) { PowerMode = mode };

        Reader(new FakeDisplay(dell)).Read().ShouldBe([new DdcReading(Dell, brightness, state)]);

        dell.BrightnessCalls.ShouldBe(brightness is null ? 0 : 1);
        _windows.Log.Take(3).ShouldBe(["capabilities " + Dell, "pause 50 ms", "power mode " + Dell]);
    }

    [Fact]
    public void A_monitor_switched_off_at_its_own_button_that_still_answers_keeps_saying_so_and_isnt_left_alone()
    {
        ...
        dell.PowerMode = 5;   // switched off, answering its power mode and nothing else
        dell.Brightness = null;
        for (var minute = 1; minute <= 12; minute++)
        {
            _clock.Advance(BrightnessReporter.ReadEvery);
            reader.Read().ShouldBe([new DdcReading(Dell, null, MonitorPowerState.Off)]);   // though its brightness falls due
        }

        dell.PowerModeCalls.ShouldBe(13);   // asked at every read: saying it is off isn't a failure
        dell.BrightnessCalls.ShouldBe(1);

        Answering(dell);   // switched back on
        _clock.Advance(BrightnessReporter.ReadEvery);
        reader.Read().ShouldBe([new DdcReading(Dell, 0.6, MonitorPowerState.On)]);   // and its brightness, long due, is asked at once
    }
    ...
    public void Each_monitor_is_asked_what_it_supports_once_then_read_every_time()
    {
        ...
        {
            reader.Read().ShouldBe([new DdcReading(Dell, 0.6, MonitorPowerState.On)]);
            _clock.Advance(DdcBrightness.BrightnessEvery);
        }
    ...
    [Fact]
    public void A_monitor_is_asked_its_power_mode_at_every_read_and_its_brightness_every_five_minutes()
    {
        var dell = new FakeMonitor(Dell);
        var reader = Reader(new FakeDisplay(dell));

        var brightnessAt = new List<int>();   // the minutes at which the monitor gave its brightness
        for (var minute = 0; minute <= 15; minute++)
        {
            var reading = reader.Read().ShouldHaveSingleItem();
            reading.Power.ShouldBe(MonitorPowerState.On);
            if (reading.Brightness is not null) brightnessAt.Add(minute);
            _clock.Advance(BrightnessReporter.ReadEvery);
        }

        brightnessAt.ShouldBe([0, 5, 10, 15]);
        dell.PowerModeCalls.ShouldBe(16);
        dell.BrightnessCalls.ShouldBe(4);
        dell.CapabilityCalls.ShouldBe(1);
        BrightnessReporter.ReadEvery.ShouldBe(TimeSpan.FromMinutes(1));
        DdcBrightness.BrightnessEvery.ShouldBe(TimeSpan.FromMinutes(5));
    }

    [Fact]
    public void The_read_due_when_a_brightness_is_five_minutes_old_asks_for_it_even_when_it_starts_a_moment_early()
    {
        // Reads are scheduled a minute apart, but the one that was given the brightness may have started a moment late.
        var dell = new FakeMonitor(Dell);
        var reader = Reader(new FakeDisplay(dell));
        reader.Read().ShouldBe([new DdcReading(Dell, 0.6, MonitorPowerState.On)]);

        _clock.Advance(BrightnessReporter.ReadEvery * 4 + TimeSpan.FromSeconds(20));
        reader.Read().ShouldBe([new DdcReading(Dell, null, MonitorPowerState.On)]);   // four minutes on, even a little late, isn't five
        _clock.Advance(BrightnessReporter.ReadEvery - TimeSpan.FromSeconds(23));
        reader.Read().ShouldBe([new DdcReading(Dell, 0.6, MonitorPowerState.On)]);   // five minutes on, less a moment
    }
    ...
    public void A_monitor_whose_power_mode_fails_three_reads_in_a_row_while_its_brightness_answers_is_taken_not_to_support_it(string failure)
    {
        ...
        for (var read = 0; read < 3; read++)
        {
            if (read > 0) _clock.Advance(BrightnessReporter.ReadEvery);
            reader.Read().ShouldBe([new DdcReading(Dell, 0.6, null)]);   // its brightness, asked each time though not due
        }
        dell.PowerModeCalls.ShouldBe(3);
        DdcBrightness.PowerModeTries.ShouldBe(3);

        // It answered its brightness each time, so it hasn't gone quiet: however long the App runs, and even if it would
        // answer now, only a display change or a resume makes the request worth sending again.
        Answering(dell);
        for (var minute = 3; minute <= 22; minute++)
        {
            _clock.Advance(BrightnessReporter.ReadEvery);
            var readings = reader.Read();
            if ((minute - 2) % 5 == 0) readings.ShouldBe([new DdcReading(Dell, 0.6, null)]);   // its brightness is due
            else readings.ShouldBeEmpty();   // nothing is due, so nothing is asked
        }

        dell.PowerModeCalls.ShouldBe(3);
        dell.BrightnessCalls.ShouldBe(7);   // its brightness is still read, every five minutes
        display.Opens.ShouldBe(7);   // and between those reads its display isn't opened
        _windows.OpenHandles.ShouldBe(0);
    }

    [Fact]
    public void A_single_power_mode_failure_is_transient_and_an_answer_ends_a_run_of_them()
    {
        var dell = new FakeMonitor(Dell);
        var reader = Reader(new FakeDisplay(dell));
        reader.Read().ShouldBe([new DdcReading(Dell, 0.6, MonitorPowerState.On)]);

        // Two failures, an answer, two more failures, an answer: never three in a row.
        uint?[] modes = [null, null, 1, null, null, 4];
        foreach (var mode in modes)
        {
            dell.PowerMode = mode;
            _clock.Advance(BrightnessReporter.ReadEvery);
            reader.Read().ShouldHaveSingleItem().Power.ShouldBe(DdcBrightness.PowerState(mode ?? 0));
        }

        dell.PowerModeCalls.ShouldBe(7);   // asked at every read throughout
    }
    ...
    public void A_display_change_or_a_resume_clears_a_power_mode_taken_as_unsupported(bool displayChange)
    {
        ...
        _clock.Advance(BrightnessReporter.ReadEvery);
        reader.Read().ShouldBeEmpty();   // its brightness isn't due, and its power mode isn't asked
        ...
        dell.PowerModeCalls.ShouldBe(4);
        dell.CapabilityCalls.ShouldBe(2);   // asked afresh, what it supports first
    }

    [Fact]
    public void A_display_change_starts_the_count_of_power_mode_failures_again()
    {
        var dell = Failing(new FakeMonitor(Dell), "power mode fails");
        var reader = Reader(new FakeDisplay(dell));
        reader.Read().ShouldBe([new DdcReading(Dell, 0.6, null)]);
        _clock.Advance(BrightnessReporter.ReadEvery);
        reader.Read().ShouldBe([new DdcReading(Dell, 0.6, null)]);   // two failures

        _events.RaiseDisplaySettingsChanged();
        _clock.Advance(BrightnessReporter.ReadEvery);
        reader.Read().ShouldBe([new DdcReading(Dell, 0.6, null)]);   // the first since the change, not the third
        _clock.Advance(BrightnessReporter.ReadEvery);
        reader.Read().ShouldBe([new DdcReading(Dell, 0.6, null)]);

        dell.PowerModeCalls.ShouldBe(4);   // so it is still asked
    }
    ...
    public void A_power_mode_that_is_no_state_is_still_an_answer_so_the_monitor_is_asked_it_again_next_time()
    {
        ...
        reader.Read().ShouldBe([new DdcReading(Dell, 0.6, null)]);
        _clock.Advance(BrightnessReporter.ReadEvery);
        reader.Read().ShouldBeEmpty();   // still no state, and its brightness isn't due
        dell.PowerMode = 4;
        _clock.Advance(BrightnessReporter.ReadEvery);
    ...
    public void A_monitor_that_has_given_a_brightness_and_then_fails_is_asked_again_at_the_next_read(string failure)
    {
        ...
        _clock.Advance(DdcBrightness.BrightnessEvery);   // its brightness is due
        reader.Read().ShouldBe([new DdcReading(Dell, null, MonitorPowerState.On)]);   // what it said before its brightness failed

        Answering(dell);   // switched back on
        _clock.Advance(BrightnessReporter.ReadEvery);
        reader.Read().ShouldBe([new DdcReading(Dell, 0.6, MonitorPowerState.On)]);   // its brightness is still due

        dell.CapabilityCalls.ShouldBe(1);   // what it supports is still known, so it isn't asked that again
        dell.BrightnessCalls.ShouldBe(3);
        dell.PowerModeCalls.ShouldBe(3);
    }
    ...
    [InlineData("power mode fails")]
    [InlineData("power mode throws")]
    public void A_monitor_whose_power_mode_and_then_brightness_fail_has_failed_the_read_and_is_asked_again_at_the_next_read(string failure)
    {
        // Between its brightness reads, a monitor whose power mode fails is asked its brightness to tell whether it still
        // answers. Failing that too, it has answered nothing, like a monitor switched off, rather than shown it doesn't
        // support power mode, so the failure doesn't count toward that.
        var dell = new FakeMonitor(Dell);
        var reader = Reader(new FakeDisplay(dell));
        reader.Read().ShouldBe([new DdcReading(Dell, 0.6, MonitorPowerState.On)]);
        ...
        {
            Failing(dell, failure);
            Failing(dell, "brightness fails");
            _clock.Advance(DdcBrightness.LongestWait);   // however long the wait
            reader.Read().ShouldBeEmpty();
        }

        Answering(dell);   // switched back on
        _clock.Advance(DdcBrightness.LongestWait);
        reader.Read().ShouldBe([new DdcReading(Dell, 0.6, MonitorPowerState.On)]);   // its power mode still asked after three

        dell.PowerModeCalls.ShouldBe(5);
        dell.BrightnessCalls.ShouldBe(5);
    }
    ...
    public void Each_failure_in_a_row_doubles_how_long_a_monitor_that_has_given_a_brightness_is_left_alone_up_to_an_hour()
    {
        ...
        Silent(dell);   // switched off at its own button, and left off

        var asked = new List<int>();   // the minutes after that first read at which the monitor is asked
        for (var read = 0; read < 300; read++)   // five hours of reads
        {
            _clock.Advance(BrightnessReporter.ReadEvery);
        ...
        // Asked at the next read and the one after, then left alone for 2, 4, 8, 16 and 32 minutes, then for an hour at a
        // time: each time its power mode, and after that fails, its brightness, which fails too.
        asked.ShouldBe([1, 2, 4, 8, 16, 32, 64, 124, 184, 244]);
        dell.PowerModeCalls.ShouldBe(1 + asked.Count);   // never taken not to support power mode, as it answered nothing
        dell.BrightnessCalls.ShouldBe(1 + asked.Count);
    ...
    public void Answering_ends_a_run_of_failures_so_the_next_failure_is_asked_again_at_the_next_read()
    {
        ...
        reader.Read().ShouldBe([new DdcReading(Dell, null, MonitorPowerState.On)]);   // its power mode is an answer too
    ...
    {
        // Reads are scheduled a minute apart, but one that starts a moment late and the next, which starts on time, are a
        // little less than that apart.
        var dell = new FakeMonitor(Dell);
        var reader = Reader(new FakeDisplay(dell));
        reader.Read().ShouldBe([new DdcReading(Dell, 0.6, MonitorPowerState.On)]);
    ...
    public void A_display_change_or_a_resume_ends_a_wait_and_the_monitor_is_asked_afresh(bool displayChange)
    {
        ...
        dell.CapabilityCalls.ShouldBe(2);   // asked afresh, what it supports first
        dell.BrightnessCalls.ShouldBe(4);   // and its brightness, though its last answer is only three minutes old
    }
    ...
    public void A_monitor_that_has_given_a_brightness_and_fails_as_it_wakes_from_a_resume_is_asked_again_at_the_next_read(string failure)
    {
        ...
        _clock.Advance(BrightnessReporter.ReadEvery);
        reader.Read().ShouldAllBe(reading => reading.Brightness == null);   // at most whether it is on, if it said that first
    ...
    public async Task A_display_change_while_a_read_waits_on_a_monitor_returns_at_once_and_the_next_read_asks_afresh()
    {
        ...
        letGo.ShouldBeTrue();
        readings.ShouldBe([new DdcReading(Lg, null, MonitorPowerState.On)]);   // the change came after the Dell's turn in that read
    ...
    public async Task A_resume_while_a_read_waits_on_a_monitor_returns_at_once_and_is_seen_before_the_next_monitor_is_asked()
    {
        ...
        var (readings, letGo) = await read;
        letGo.ShouldBeTrue();
        // The Dell's turn came after the resume.
        readings.ShouldBe([new DdcReading(Lg, null, MonitorPowerState.On), new DdcReading(Dell, 0.6, MonitorPowerState.On)]);
        dell.CapabilityCalls.ShouldBe(2);
    }
    ...
    [Fact]
    public void A_monitor_is_given_50_ms_between_one_request_and_the_next()
    {
        var dell = new FakeMonitor(Dell);
        var reader = Reader(new FakeDisplay(dell));
        ...
        _clock.Advance(BrightnessReporter.ReadEvery);
        reader.Read();
        _clock.Advance(DdcBrightness.BrightnessEvery - BrightnessReporter.ReadEvery);
        dell.PowerMode = null;
        reader.Read();
        ...
        [
            "capabilities " + Dell, "pause 50 ms", "power mode " + Dell, "pause 50 ms", "brightness " + Dell,
            "power mode " + Dell,   // a read's first request follows none, so nothing waits before it
            "power mode " + Dell, "pause 50 ms", "brightness " + Dell,   // after a request that failed too
        ]);
    }
    ...
    public void A_monitor_left_alone_is_known_by_its_device_path_whatever_handle_it_gets_next()
    {
        ...
        reader.Read().ShouldBe([new DdcReading(Lg, 0.6, MonitorPowerState.On)]);
        _clock.Advance(BrightnessReporter.ReadEvery);
        reader.Read().ShouldBe([new DdcReading(Lg, null, MonitorPowerState.On)]);
    ...
    [Fact]
    public void A_monitor_that_reports_no_range_gives_no_brightness_and_is_asked_again_when_its_brightness_is_next_due()
    {
        ...
        _clock.Advance(BrightnessReporter.ReadEvery);
        reader.Read().ShouldBe([new DdcReading(Dell, null, MonitorPowerState.On)]);   // it answered, so it isn't asked again at once
        _clock.Advance(DdcBrightness.BrightnessEvery - BrightnessReporter.ReadEvery);
    ...
    public void Handles_are_destroyed_after_a_call_throws()
    {
        ...
        reader.Read().ShouldBe([new DdcReading(Dell, null, MonitorPowerState.On), new DdcReading(Lg, 0.6, MonitorPowerState.On)]);
    ...
    public void This_machines_monitors_are_read_without_throwing_and_every_brightness_is_between_0_and_1()
    {
        ...
        var readings = reader.Read().Concat(reader.Read()).ToList();   // the second asks those that answered the first only their power modes
    ...
    /// <paramref name="slow"/>'s first request, where it stays until <paramref name="answer"/> is set. The task gives the
    /// readings, and whether the monitor was let go that way rather than giving up after <see cref="Timeout"/>.</summary>
    ...
    /// <summary>The monitor answers nothing, as many do once switched off at their own button.</summary>
    private static void Silent(FakeMonitor monitor)
    {
        monitor.Capabilities = null;
        monitor.Brightness = null;
        monitor.PowerMode = null;
    }
    ...
    private sealed class FakeMonitor(string path, string description = "Generic PnP Monitor")
    {
        ...
        /// <summary>Runs inside each request the monitor is sent, before it answers, so a test can hold a read there.</summary>

 ...
 * key; the per-address rate limit (an IPv6 address by its /64); gzip, at most 1 MB as sent, counted while reading so
 * an oversized body is never held whole; bounded decompression (8 MB); then JSON. `body` is the bytes as sent.
 */

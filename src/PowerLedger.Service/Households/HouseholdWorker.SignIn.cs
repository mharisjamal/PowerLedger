...
/// <summary>N2's sign-in (households design §7, plan C11), on the household worker.</summary>
internal sealed partial class HouseholdWorker
{
    ...
    /// in, it asks to join; with none and this PC in one, the account is linked to it, and a recovery code made and given
    /// back once.
    /// </summary>
    ...
    /// <summary>Makes the recovery code for the household, puts the envelope with the account and keeps the code's key.</summary>
    /// <returns>The code, to be shown once; null when the envelope couldn't be put.</returns>
    private async Task<string?> MakeRecoveryAsync(string session, string householdId, CancellationToken cancel)
    {

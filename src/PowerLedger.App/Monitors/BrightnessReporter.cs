...
/// runs in the user's. A minute after the App starts and every minute after, while the user allows it, it asks the service
/// which monitors it knows, reads them off the UI thread - each one's power mode every time, and its brightness once its
/// last is five minutes old (<see cref="DdcBrightness"/>) - and reads the refresh rate and HDR state Windows drives each at
/// (<see cref="DisplayConfigReader"/>). It reports each monitor the service listed, under the service's own name for it,
/// with whatever was read: a brightness, a power state, how it is driven, or any of them. It reads nothing when the service
/// lists no monitors, or when the service's latest reading says the displays are off, and it sends nothing to a service
/// from before monitors, which lists none and drops a connection that sends it a message it doesn't know. Nothing here
/// throws: a report that fails is simply made again at the next read.
/// </summary>
internal sealed class BrightnessReporter(IServiceLink link, IBrightnessReader reader, IDisplayReader displays, IUiSettings ui, TimeProvider clock) : IDisposable
{
    ...
    internal async Task ReportAsync()
    {
        ...
        try
        {
            ...
            var (brightness, power) = Answered(reader.Read(), instances);

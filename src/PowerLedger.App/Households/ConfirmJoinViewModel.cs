...
internal sealed class ConfirmJoinViewModel : ObservableObject
{
    ...
    /// withdrawn (service round, review: an unanswered ConfirmJoin can come back under a new PromptId too, and this PC
    /// never shows two windows for one request). Stops the timer for good, so it can never answer late; sends nothing,
    /// since this PC isn't the one closing it.</summary>
    public void Stop()
    {
        _answered = true;
        _timer?.Dispose();

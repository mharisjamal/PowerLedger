...
reports over USB is read with read commands only. A Corsair reports what it draws from the wall, and that is taken as it
is; the others report what their rails put out, which is divided by the supply's efficiency at the load it is carrying,
read from its 80 PLUS tier and the rating in its model name. A supply idling at a fraction of what it is rated for is
well short of the efficiency its tier certifies, so an idle desktop reads higher than a flat tier figure would say.
PowerLedger gives way to iCUE, CAM or Thermaltake's app while one of them is running,
